...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-time-report-reads/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) TimeReportCollection(iD string) {
	data, err := c.callTimeReportSrvAPIRequirementTimeReportCollection("CodTimeReportCollection", iD)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) TimeReportPartyCollection(objectID, partyID string) {
	data, err := c.callTimeReportSrvAPIRequirementTimeReportPartyCollection("CodTimeReportPartyCollection", objectID, partyID)
	if err != nil {
		c.log.Error(err)
		return
	}
